...
@click.option(
    "--limit",
    help=f"The maximum number of blob objects to return (defaults to 100)", type=click.INT, default=100
)
@click.option(
    "--prefix",
    help=f"A string used to filter for blob objects contained in a specific folder assuming that the folder name was used in the pathname when the blob object was uploaded", type=click.STRING
)
@click.option(
    "--cursor",
    help=f"A string obtained from a previous response for pagination of results", type=click.STRING
)
@click.option(
    "--mode",
...
def head(ctx: click.Context, url:str):
    resp = blob.head(url, options={"token": ctx.obj["token"], "debug":ctx.obj["debug"]})
    if ctx.obj["print_json"]:
        click.echo(resp)
    else:
        click.echo(tabulate([(k,v) for k,v in resp.items()]))
...
def copy(ctx: click.Context, from_url:str, to_pathname:str):
    resp = blob.copy(from_url, to_pathname, options={"token": ctx.obj["token"], "debug":ctx.obj["debug"]})
    if ctx.obj["print_json"]:
        click.echo(resp)
    else:
        click.echo(tabulate([(k,v) for k,v in resp.items()]))
